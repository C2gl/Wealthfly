...
## Later / backlog
- [ ] **Incremental / Streamed Sync**: Support streaming or incremental sync (`updated_at_after`) to avoid buffering all transactions in memory via `fetchAllPages` for multi-year ledgers.
...
- **Memory on Large Ledgers**: `fetchAllPages` accumulates all transaction groups into a single JavaScript array in memory before insertion. For ledgers spanning 5–10 years, this should be paged directly into SQLite batches.
